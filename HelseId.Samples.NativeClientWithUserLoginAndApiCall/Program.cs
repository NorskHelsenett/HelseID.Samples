...
namespace HelseId.Samples.NativeClientWithUserLoginAndApiCall
{
    class Program
    {
        ...
        private const string _apiUrl = "https://helseidhackatonapi.azurewebsites.net/api";

        // The port do not need to be preregistred in HelseID, so you can allocate an available port when launching the application
        private static string _localhost = "http://localhost:8089";

        private static string _redirectUrl = "/callback";
        private static string _startPage = "/start";

        static async Task Main(string[] args)
        {
            try
            {
                // Used the available information at https://helseid-sts.test.nhn.no/.well-known/openid-configuration to determine endpoints and public keys used by HelseID
                var disco = await GetInformationAboutHelseIdFromMetadata(_stsUrl);

                // This is the scope of the API you want to call (get an access token for)
                var apiScopes = "nhn:helseid-public-samplecode/authorization-code";
                ...
                var identityScopes = "openid profile helseid://scopes/identity/pid helseid://scopes/identity/security_level";
                ...
                {
                    Authority = _stsUrl,
                    RedirectUri = _localhost + _redirectUrl,
                ...
                var response = await Authorize(_localhost, _redirectUrl, _startPage, state);
                ...
                // a web browser
                var clientAssertionPayload = GetClientAssertionPayload(disco, _clientId);
