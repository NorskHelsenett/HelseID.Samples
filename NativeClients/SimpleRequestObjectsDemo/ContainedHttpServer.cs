    ...
    public class ContainedHttpServer : IDisposable
    {
        ...
        private void SetResult(string value, HttpContext ctx)
        {
            try
            {
                ...
                ctx.Response.WriteAsync("<h3>You can now return to the application.</h3>");
                ctx.Response.WriteAsync("<p>The ID token and access token (after the initial login), and a refreshed access token will be presented in the terminal.</>");
