 ...
 */
@ApiModel(description = "Information regarding false identity on a person  An identity belonging to another person then the person using it.  Or not belonging to anyone at all, but presented as a real identity                Persons who is currently or have had their NIN (Norwegian Identification Number)  misused by others, will not have information about the misuse.  Neither will there be info on the person misusing another persons identity.  <br>FREG: FalskIdentitet")
...
public class FalseIdentity {
  ...
  * @throws IOException if the JSON Object is invalid with respect to FalseIdentity
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (FalseIdentity.openapiRequiredFields.isEmpty()) {
          return;
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FalseIdentity.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FalseIdentity` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
